...
class ScreenTab(Button):
    def __init__(self, x, id, app):
        super().__init__(x, SIZE[1]-50, [Renderable(pygame.Rect(x, SIZE[1]-50, SIZE[0]/4, 50), VERY_DARK_GREEN, 0), Renderable(pygame.transform.scale(loader.load_image(f"icons/{id}"), (35, 35)), (x+SIZE[0]/8-35/2, SIZE[1]-50+25-35/2))], lambda:None, lambda:None, lambda:None, self.click)
        self.id = id
        self.app = app
    def click(self):
